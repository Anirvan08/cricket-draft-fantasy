...
export async function getPicksByLeague(supabase, leagueId) {
  const { data, error } = await supabase
    ...
    .select(`
      *,
      player:players!draft_picks_player_id_fkey(*),
...
export async function replacePick(supabase, { pickId, newPlayerId, reason }) {
  const { error } = await supabase.rpc('replace_draft_pick', {
    p_pick_id:       pickId,
